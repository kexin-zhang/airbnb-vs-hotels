# example usage: python scrape.py "2018/01/01" "2018/01/02" "popularity" "boston"
#!/usr/bin/env python
...
def parse(locality,checkin_date,checkout_date,sort):
    ...
    for hotel in hotel_lists:
        ...
        XPATH_REVIEWS  = './/span[contains(@class, "review_count")]//text()'
        ...
        XPATH_HOTEL_PRICE = './/div[contains(@class, "available")]/@data-pernight'
...
if __name__ == '__main__':
    ...
    if today<datetime.strptime(checkIn,"%Y/%m/%d") and datetime.strptime(checkIn,"%Y/%m/%d")<datetime.strptime(checkOut,"%Y/%m/%d"):
        ...
        with open('tripadvisor_data.csv','w')as csvfile:
            fieldnames = ['hotel_name','url','locality','reviews','tripadvisor_rating','checkIn','checkOut','price_per_night','booking_provider', 'hotel_features']
